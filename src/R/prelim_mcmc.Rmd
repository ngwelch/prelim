...
results = tmp[5000:nrow(fullChain),]
...
```{r}
p = ggplot(results) + geom_path(aes(x=1:number_of_trials, y=mu))+theme_bw()
p
...
```{r}
p = ggplot(results) + geom_path(aes(x=1:number_of_trials, y=theta))+theme_bw()
p
...
```{r}
p = ggplot(results) + geom_path(aes(x=1:number_of_trials, y=sigma))+theme_bw()
p
...
### Convergence Tests
```{r}
MCMC.chain1 = mcmc(data=results[,c('mu', 'sigma', 'theta')], 
                   start = 1, end = nrow(results), thin = 1)
plot(MCMC.chain1, trace = T, density = TRUE, smooth = TRUE)
```
...
```{r}
cumuplot(MCMC.chain1, probs=c(0.025,0.5,0.975))
```
```{r}
autocorr(MCMC.chain1, lags = c(0, 1, 5, 10, 50, 100))
autocorr.plot(MCMC.chain1, lag.max=100, auto.layout = TRUE, ask = TRUE)
```
```{r}
effectiveSize(MCMC.chain1)
geweke.diag(MCMC.chain1, frac1=0.1, frac2=0.5)
raftery.diag(MCMC.chain1, q=0.95, r=0.01, s=0.95, converge.eps=0.001)
```
...
```
```{r, echo=FALSE}
prior_scale = rgamma(10000, shape=0.7, scale=0.004)
prior_rate = rgamma(10000, shape=0.7, rate=0.7)

thewidth=0.5
thexlim = c(0, 0.008)
plot(NA, xlim=thexlim, ylim=c(0,2),
		xlab=expression(mu), yaxt="n", ylab="")
axis(2, at=c(0.5, 1.5), labels=c("Rate", "Scale"), las=2)

denstrip(prior_scale, horiz=T, at=1.5, 
		ticks=quantile(prior_scale, probs=c(0.025, 0.5, 0.975)), width=thewidth)

denstrip(prior_rate, horiz=T, at=0.5,
		ticks=quantile(prior_rate, probs=c(0.025, 0.5, 0.975)), width=thewidth)
...
```{r}
png(filename="/Users/nwelch/prelim/report/figures/figure_2.png", 
    res=300, width=3, height=5, units='in')
...
fig5Labels = read.csv(file="/Users/nwelch/prelim/data/fig5ProbLabels.csv")
...
apply(cbind(colLabel, colGrey), 1, function(s) paste0(s[1], "=", s[2]))
thecol = c("zeroTo19"="#FFFFFF", "twentyTo79"="#A6A6A6", 
           "eightyTo99"="#595959", "knownInfection"="#0D0D0D")
