...
```{r}
plot(MCMC.chain, trace = T, density = TRUE, smooth = TRUE)
```

```{r}
chainTail = (number_of_trials-1e4):number_of_trials
cumuplot(MCMC.chain[chainTail,], probs=c(0.025,0.5,0.975))
```
```{r}
autocorr(MCMC.chain, lags = c(0, 1, 5, 10, 30))
autocorr.plot(MCMC.chain, lag.max=30, auto.layout = TRUE, ask = TRUE)
```
```{r}
