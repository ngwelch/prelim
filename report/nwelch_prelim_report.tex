...
Data augmentation is a common approach to overcoming \vmdel{this inference with} 
\vmadd{the problem of} unknown infection times. 
In this case, unknown infection times are \vmdel{modeled} \vmadd{treated} as latent variables. 
...
\citet{Jewell} discusse\vmdel{s} an augmentation approach in the context of a \textit{Metropolis-in-Gibbs} MCMC framework and the simulation setup described in the next section follows directly \vmadd{from their work}. 
...
As a result, let the function $\theta f(x_i -x_j \vmdel{\sigma |} \vmadd{; } \sigma)$ denote the rate of infection for plant $i$ at time $t$ after plant $j$ was infected prior to $t$. 
...
\begin{equation} 
L(\theta, \mu, \tau) = \left[ \prod_{i;\tau_i \le T} \exp \left\{-\int^{\tau_i}_0 \lambda(x_i, t)dt\right \} \lambda(x_i, \tau_i)\right] \left[ \prod_{i;\tau_i > T} \exp \left\{-\int^{\tau_i}_0 \lambda(x_i, t)dt \right\}\right] \vmadd{.}
\end{equation}
...
The last component of the model to specify is the kernel function, $f(x_i -x_j \vmdel{|} \vmadd{;} \sigma)$. 
In this case, a radially symmetric bivariate Gaussian density is appropriate. 
\begin{equation} f( x_i-x_j \vmdel{|} \vmadd{;}\sigma) = \frac{1}{2\pi \sigma^2} \exp \left\{-\frac{ ||x_i-x_j||^2}{2\sigma^2} \right\} \vmadd{.}
