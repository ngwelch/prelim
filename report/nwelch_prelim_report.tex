...
\newcommand{\blueline}{\raisebox{2pt}{\tikz{\draw[-,blue,dashed,line width = 2pt](0,0) -- (8mm,0);}}}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{(\subref{fig:plants_locations}) Plant locations (meters), infection status (\sqbox{plotBlack} infected) after 30 weeks, and sample data used to fit a model (\protect\blueline); (\subref{fig:cum_infection}) number of infected plants at each inspection time. 
...
\begin{enumerate}
\itemsep0em
\item Initialize the algorithm at $\tau_i^{0}$, $\mu^{0}$, $\sigma^{0}$, $\theta^{0}$ for iteration $r=0$\vmadd{,}
\item at iteration $r$, set $\tau_i^{(r)}=\tau_i^{(r-1)}$, $\mu^{(r)}=\mu^{(r-1)}$, $\sigma^{(r)}=\sigma^{(r-1)}$, $\theta^{(r)}=\theta^{(r-1)}$\vmadd{,}
\item simulate a proposal $\mu^{\star} \sim N(\mu^{(r-1)}, \nu_{\mu})$\vmadd{,}
\item set $\mu^{(r)} = \mu^{\star}$ with probability
$$\min \left\{1, \frac{L( \tau^{(r)}_1, \dots, \tau^{(r)}_N \mid \mu^{\star}, \theta^{(r)}, \sigma^{(r)} ) \; p_{\mu}(\mu^{\star})}{L(\tau^{(r)}_1, \dots, \tau^{(r)}_N \mid \mu^{(r)}, \theta^{(r)}, \sigma^{(r)} ) \; p_{\mu}(\mu^{r})} \right\}\vmadd{,} $$
			otherwise, set $\mu^{(r)}=\mu^{(r-1)}$\vmadd{,}
\item repeat steps 3 and 4 for $\theta$ and $\sigma$\vmadd{,}
\item for each $i=1, \dots, N$, propose a new $\tau^{\star}_i$ and accept with probability
$$\min \left\{1, \frac{L( \tau^{(r)}_1, \dots, \tau^{(r)}_{i-1}, \tau^{\star}_i,\tau^{(r)}_{i+1}, \dots, \tau^{(r)}_N \mid \mu^{(r)}, \theta^{(r)}, \sigma^{(r)} )}{L(\tau^{(r)}_1, \dots, \tau^{(r)}_N \mid \mu^{(r)}, \theta^{(r)}, \sigma^{(r)} )} \right\}\vmadd{,} $$
\item return to step 2 until a sufficiently large sample has been obtained\vmadd{.}
\end{enumerate}
}
The \vmadd{algorithm} implementation for this model uses normally distributed proposals with means at the previous $\theta$, $\sigma$, $\mu$, and $\tau_i$ values. 
...
This post-processed sample is what is shown in \vmdel{f}\vmadd{F}igure \ref{fig:muSigmaThetaDensities}. 

Comparing \vmadd{Figure} \ref{fig:mu_density} to the same result reported in \citep{Brown} suggests some discrepancies in the results. 
...
If a highly informative prior was used, then the density plot in \citet{Brown}'s \vmdel{f}\vmadd{F}igure 2a should reflect this fact. 
...
The results in \vmdel{f}\vmadd{F}igure (\ref{fig:theta_density}) and (\ref{fig:sigma_density}) are consistent with the the authors' findings, but finding the right proposal \vmdel{concentration} \vmadd{tuning} parameters to obtain convergence was a challenge and required several trial runs. 
...
\begin{figure}[H]
	...
	\end{subfigure} 
	\caption{(\subref{fig:mu_density}) - (\subref{fig:sigma_density}) show\vmdel{s} the parameter \vmadd{posterior} densities;  (\subref{fig:muSigma}) - (\subref{fig:sigmaTheta}) shows pairwise posterior \vmdel{samples} \vmadd{pairwise scatter plots} }
...
This slopes of the lines here are less than the results reported \vmdel{in} \vmadd{by} \citet{Brown} for two reasons. 
...
Figure \ref{fig:sim_epidemic} shows simulated epidemics that correspond to the 165 plant sample enclosed in the blue box in \vmdel{f}\vmadd{F}igure (\ref{fig:data_plot}) \vmcomment{You are using the same figure label multiple times --- fix it}. 
...
This follows the basic conclusion in \citet{Brown}, but the predicted infection probabilities reported there were coded \vmdel{as} so that the first infection time after week 30 arises from the endemic rate instead of the combined influences of the endemic rate and all previously infected plants.
