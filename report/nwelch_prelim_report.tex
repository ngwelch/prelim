...
This report offers corrections to these and other errors, offers a more rigorous formulation of the statistical model and priors specified by \citet{Brown}, and assesses convergence issues inherent with high-dimensional parameter spaces. 
\end{abstract}
...
Failing to adjust this rate leads to a rate 30 times too large. 
See the Results section for more information on the the effects of these errors and Appendix \ref{coding_corrections} for a detailed discussion of the evidence for the error. 
...
\citet{Brown} suggest deriving the $\theta$ prior using the average time to the first infection in a ring of healthy plants encircling an infected plant. 
...
The authors suggest a prior where the average time to infection for a specific plant scenario falls between 1 day at the 2.5\% quantile and 16 months (interpreted as 480 days) at the 97.5\% quantile.
...
While $\theta \sim \Gamma(0.8, \text{rate}=0.1)$ led to reasonable coverage results with small sample sizes, the parameters seemed less compelling under simulations with larger samples.
...
The actual acceptance rates were 27\% for $\mu$, 30\% for $\sigma$, and 29\% for $\theta$. 
...
Computation times in early prototypes improved with each suggestion implemented, but time to complete prototyping runs was still prohibitively long with the full data set. 
Taking a 10\% sample of the full data set substantially reduced prototyping times; it also led to the discovery of bugs in \citep{Brown}'s implementations.
...
The first 30,000 iterations were discarded as burn-in trials based on inspection of the parameter trace plots and Geweke's diagnostic. 
...
The results in Figure (\ref{fig:theta_density}) and (\ref{fig:sigma_density}) are consistent with the the authors' findings, but as with the distribution of $\mu$, the 95\% confidence intervals are wider than what the original paper due, in part, to sampling the full data set. 
...
The relationship between $\mu$ and $\sigma$ shown in Figure (\ref{fig:muSigma}) suggests that the spatial influence tends to diminish as the the endemic infection rate falls. 
This offers some indication that the spatial parameter must be relaxed in order for the model to account for new infections far from an infected plant. 
...
Figure (\ref{fig:sigmaTheta}) offers one of the clearest indications that the posterior distribution of $\sigma$ is right skewed. 
...
These plots are based on a 25\% sample of the data and as a result lead to less informative interpretation than a larger or the full data set. 
The figures do, however, indicate that the posterior parameters sampled lead to more infections earlier than what was observed in the sample data. 
...
This is consistent with \citet{Brown}'s findings with the full data set and correcting the bug on the $\mu$ prior was not enough to change the inference for the first few weeks of a new outbreak. 
...
This result follows from the wider confidence intervals associated with the sampled data; however, it is not clear how much of this additional variation can be explained by sampling the data versus correctly parameterizing the prior $\mu$ prior. 
...
Each panel shows the predicted infection probabilities for each plant starting from the infection status of plants starting with the infections recorded at week 30. 
...
Considering that the authors' simulations suggested approximately 40 new infections per week under the model parameters sampled, having most of the plants remain healthy after 10 weeks time proves the claim. 
With this rate of infection, one should expect the simulations to suggest around 400 new infections by week 40 in addition to the 583 infections known to exist in week 30. 
...
In addition to inference for ILMs, improved computing times were a central result reported by the authors.
