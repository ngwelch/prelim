...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{(\subref{fig:plants_locations}) Plant locations (meters), infection status (black=infected) after 30 weeks, and sample data used to fit a model (blue); (\subref{fig:cum_infection}) number of infected plants at each inspection time. 
...
\begin{equation} 
L(\theta, \mu, \tau) = \left[ \prod_{i;\tau_i \le T} \exp \left\{-\int^{\tau_i}_0 \lambda(\boldsymbol{x}_i, t)dt\right \} \lambda(\boldsymbol{x}_i, \tau_i)\right] \left[ \prod_{i;\tau_i > T} \exp \left\{-\int^{\tau_i}_0 \lambda(\boldsymbol{x}_i, t)dt \right\}\right] .
\end{equation}
...
\vmcomment{Just noticed, you keep using $\boldsymbol{x}_i - \boldsymbol{x}_j$ without defining it. 
Are $\boldsymbol{x}$s two-dimensional? If yes, why aren't you using Euclidean distance?}
