...
The authors offer mathematical and computational insights to improve computation time, but errors in the published Metropolis algorithm source code led the authors to inferences and conclusions unsupportable by the data. 
\end{abstract}
...
\citet{Brown} suggests weakly informative priors for the endemic infection rate ($\mu$), the epidemic infection rate ($\theta$), and the variance for distance disease travels in a week ($\sigma^2$). 
According to the authors, prior parameters were specified using 95\% prior prediction intervals for each parameter. 
While a portion of the prior specification was described thoroughly enough to approximate the prior specification process described, others were less direct and multiple typographic errors added compounded the challenge to replication. 
Consider each suggested specification approach and resulting distribution in turn. 

\subsubsection{$\boldsymbol{\mu}$ Prior Predictive Distribution Specification}
The endemic infection rate influences the number of spontaneous infects over a specified time period. 
While spontaneously infected plants have the capacity to produce new infections in the model specified above, consider the case where only endemic infections are possible.
Focusing on this scenario will lead to simplifications that make it possible to solve for the prior parameters. 

\citet{Brown} proposes a $\Gamma(\alpha, \beta)$ prior for the endemic infection rate, $\mu$, and suggests that the appropriate prior predictive distribution should lead to 1 infection at the 2.5\% and 630 at the 97.5\% prior predictive distribution quantiles. 
If epidemic infections are not possible, then these bounds are plausible considering that the number of plants infected after 30 weeks time was 583. 
Starting with the probability of observing $k$ infections, the prior predictive distribution can be rewritten as 
\begin{equation}
P(k) = \int_0^{\infty} P(k, \mu) d\mu = \int_0^{\infty} P(k \mid \mu)  P(\mu) d\mu
\label{eq:prior_predictive}
\end{equation}
While only the first infection is recorded, it is possible for plants to be infected multiple times. 
This suggests the sampling model $k \mid \mu \sim$ Poisson($\mu$) is not only convenient, but also appropriate. 
Therefore, 
\begin{align*}
...
The differences are, however, significant when considering the prior predictive quantiles and variances corresponding to the distribution $\mu_{\text{Brown}} \sim \Gamma(0.7, 0.004)$ and $\mu_{\text{replicated}} \sim \Gamma(0.6893886, 0.004766281)$. 
...
Replicating the technique described by the authors also leads to a slightly more informative prior based on the variances of the two distributions. 

\textbf{[Remaining prior analysis left for later... Moving on to inference]}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.36\linewidth, keepaspectratio]{/Users/nwelch/prelim/report/figures/brown_figure_4.png}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.26\linewidth, keepaspectratio]{/Users/nwelch/prelim/report/figures/brown_figure_2a.png}
...
\begin{figure}[H]
\centering
\includegraphics[height=0.6\linewidth, keepaspectratio]{/Users/nwelch/prelim/report/figures/figure_3.png}
\caption{tbd...}
\label{fig:prior_mu_plot}
\end{figure} 
...
\begin{figure}[H]
\centering
\includegraphics[height=0.6\linewidth, keepaspectratio]{/Users/nwelch/prelim/report/figures/figure_4.png}
\caption{tbd...}
\label{fig:prior_mu_plot}
\end{figure} 
...
\begin{figure}[H]
\centering
\includegraphics[height=0.51\linewidth, keepaspectratio]{/Users/nwelch/prelim/report/figures/figure_5.png}
\caption{tbd...}
\label{fig:prior_mu_plot}
\end{figure} 
